...
window.touchbanner = {};
/*封装一个事件 过渡结束事件*/
touchbanner.transitionEnd = function (dom, callback) {
    //1.给谁加事件
    //2.事件触发后处理什么业务
...
//封装一个tap事件
touchbanner.tap = function (dom, callback) {
    if (!dom || typeof dom != 'object') {
        //没dom的时候或者不是一个对象的时候 程序停止
        return false;
    }
...
window.onload = function () {
    ...
    var imageCount = $(".clearfix li").length; //页面中用来轮播的图片有5张不同的
    //轮播图大盒子
    ...
    //等过渡结束之后来做无缝衔接
    touchbanner.transitionEnd(imageBox, function () {
        //处理事件结束后的业务逻辑
        if (index >= imageCount) {
            index = 0;
        } else if (index < 0) {
