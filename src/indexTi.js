    ...
    await page.evaluate((searchItems) => {
      ...
      async function changeTiSelect() {
        ...
        if (tiSelects && tiSelects.length == 3) {
          for (var i = 0; i < tiSelects.length; i++) {
            ...
            // await sleep(100);
            await eleClick(select.children[i]);
          }
        }
      }
      ...
      async function quickAddToCartByFile() {
        ...
        var removeRowButton = document.querySelector(
          ".ti-quick-add-to-cart-worksheet-remove-bom ti-button"
        );
        if (removeRowButton) {
          //删除原来的文件
          ...
          await sleep(800);
          if (
            document.querySelector(".ti-dialog-visible") &&
            document
              .querySelector(".ti-dialog-visible")
              .querySelector("ti-button")
          ) {
            await eleClick(
              document
                .querySelector(".ti-dialog-visible")
        ...
        document.querySelector("ti-drag-and-drop").dispatchEvent(
          new CustomEvent("tiFileUploaded", {
            bubbles: true,
            composed: true,
            cancelable: true,
            detail: new File(
              [fileContent.join("\n")],
              "Quick add to cart template.csv",
        ...
        var actionButton = document.querySelector(
          ".ti-quick-add-to-cart-worksheet-actions ti-button"
        );
        await sleep(500);
        while (
          !actionButton ||
          actionButton.classList.contains("ti-button-disabled")
        ) {
        ...
        while (true) {
          var tr = document.querySelector(
            ".ti-quick-add-to-cart-worksheet-table tbody tr"
          );
        ...
        if (
          document
            .querySelector(".ti-quick-add-to-cart-worksheet-actions ti-button")
            .classList.contains("ti-button-disabled")
        ) {
          if (
            document.querySelectorAll(".ti-quick-add-to-cart-error").length ==
            30
          ) {
            console.info("均无库存1");
          ...
          var errBtn = document.querySelectorAll(
            ".ti-quick-add-to-cart-worksheet-start-row ti-button"
          )[1];
